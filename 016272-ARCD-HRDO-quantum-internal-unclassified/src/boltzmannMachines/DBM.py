...
class DBM(nn.Module):
    ...
    def evaluateQBM(self,state):
        ...
        uState,iI = state.unique(return_inverse=True,dim=0)

        n_cases = uState.shape[0]
        ...
        for iC in range(n_cases):
            self.Hamiltonians[iC],self.Hamiltonian_tensors[iC] = self.buildHamiltonian(uState[iC,:])       
    ...
    def loadWeights(self,Location):
        ...
        self.cBias = nn.Parameter(torch.from_numpy(np.loadtxt(os.path.join(Location,'cBias.txt'))).float())
