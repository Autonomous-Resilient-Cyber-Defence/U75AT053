...
class DBM_Hypernet(DBM_action):
    ...
    def loadNN(self,NN,trainEpochs=0,storeFrequency=0,storeLocation=None,learningRate=0.0001):
        ...
        with torch.no_grad():
            meanStd = torch.std(self.nn[0][2].bias).detach()
            self.hvWeights[:] = self.hvWeights / meanStd
            self.hhWeights[:] = self.hhWeights / meanStd
            self.vBias[:] = self.vBias / meanStd
            self.hBias[:] = self.hBias / meanStd
        ...
        for iE in range(trainEpochs):
            ...
            for iO in range(n_observations):
                optimizer.zero_grad()
                self.getNN(scale = False,slice=iO,forceUpdate=True)
    ...
    def getNN(self,scale=True,slice=None,forceUpdate=False):
