...
from primaite.common.enums import SessionType
...
def convertModel_syntheticData(newModelCase,newCaseI,training_config_location,lay_down_config,
    ...
    session_new._agent_session._agent.policy.optimizer.param_groups[0]['lr'] = session_new._agent_session._agent.policy.optimizer.param_groups[0]['lr'] * 10
    ...
    for iE in range(epochs):
        ...
        if iE % save_frequency == 0:
            ...
            load_agent.save_agent_session(session_new,session_new.session_path.joinpath(f'learning\\checkpoints\\epoch_{iE:.0f}\\'))
    ...
    session_new._training_config.session_type = SessionType.EVAL
    session_new.evaluate()           
...
def evaluateModel(training_config_location,lay_down_config,modelCase,caseI,model_location,num_episodes=10):
    # Convert paths to path objects and join
    if not modelCase is Path:
        modelCase = Path(modelCase)
    if not training_config_location is Path:
        training_config_location = Path(training_config_location)
    if not model_location is Path:
        model_location = Path(model_location)
    folder = Path.joinpath(training_config_location,modelCase)

    # Get config files for new and old model
    config = [case for case in folder.glob('*.yaml')][caseI]
    # Load in agent
    session = load_agent.load_agent_session(config,lay_down_config,model_location)   
    session._agent_session._training_config.num_eval_episodes = num_episodes
    session._training_config.session_type = SessionType.EVAL
    session.evaluate()           
