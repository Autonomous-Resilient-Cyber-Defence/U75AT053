...
#Importing MappingProxyType to serve as an immutable dictionary
...
class TrainingConfig:
    ...
    DBM_class_parameters: dict = field(default_factory=list)
    "The parameter dictionary for the DBM class"

    PPO_class_parameters: dict = field(default_factory=list)
    "The parameter dictionary for the sb3's PPO class"
...
def get_dbm_parameters(file_path: Union[str, Path]) -> dict:
    """
    extract the dbm class parameters from the training config yaml file
    :param file_path: The config file path.
    """
    with open(file_path, "r") as file:
        config = yaml.safe_load(file)
    return config['DBM_class_parameters']

def get_ppo_parameters(file_path: Union[str, Path]) -> dict:
    """
    extract the ppo class parameters from the training config yaml file
    :param file_path: The config file path.
    """
    with open(file_path, "r") as file:
        config = yaml.safe_load(file)
    return config['PPO_class_parameters']
