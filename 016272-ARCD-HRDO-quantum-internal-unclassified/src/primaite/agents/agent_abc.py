...
class AgentSessionABC(ABC):
    ...
    def __init__(
        ...
        :param previous_agent_path: path of previous agent.
        """
        ...
        self.previous_agent_path = previous_agent_path
        self.agent_to_evaluate_path = agent_to_evaluate_path
        ...
        if session_path is not None:
            ...
            #Setting the DBM parameters in a dict, we will use this to write DBM info to the session_metadata.json file
            self.DBM_class_parameters_dict = training_config.get_dbm_parameters(self._training_config_path)
    ...
    def _write_session_metadata_file(self) -> None:
        ...
        """
        #JS Notes: repo is an instance of the Repo class that represents your repository
        #JS Notes: sha is the hash of the commit that HEAD points to
        repo = git.Repo(search_parent_directories=True)
        sha = repo.head.object.hexsha
        try: #JS11: fixing '''SB3Agent' object has no attribute 'DBM_class_parameters_dict''' bug when loading a session
            DBM_class_parameters_dict = self.DBM_class_parameters_dict
        except:
            DBM_class_parameters_dict = None
        metadata_dict = {
            "uuid": self.uuid,
            "start_datetime": self.session_timestamp.isoformat(),
            "end_datetime": None,
            "DBM_parameters_from_training_config": DBM_class_parameters_dict,
