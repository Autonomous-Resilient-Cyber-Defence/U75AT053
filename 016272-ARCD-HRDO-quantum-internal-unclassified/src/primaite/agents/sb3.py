...
class SB3Agent(AgentSessionABC):
    ...
    def __init__(
        ...
        agent_to_evaluate_path: Optional[Union[str, Path]] = None,
    ) -> None:
        """
        ...
        super().__init__(
            training_config_path, lay_down_config_path, session_path, legacy_training_config, 
            legacy_lay_down_config, previous_agent_path, agent_to_evaluate_path
        )
    ...
    def _setup(self) -> None:
        ...
        # JS11: check if there is an agent to evaluate
        if self.agent_to_evaluate_path is not None:
            if not isinstance(self.agent_to_evaluate_path, Path):
                self.agent_to_evaluate_path = Path(self.agent_to_evaluate_path)
            agent_to_evaluate_file = next(self.agent_to_evaluate_path.rglob("*.zip"), None)
        else:
            agent_to_evaluate_file = None
        ...
        #we use agent_to_evaluate to load an agent to be used in eval mode.
        if not load_file and not agent_to_evaluate_file:
            ...
            if previous_agent_load_file is not None:
                ...
                #JS11: shouldn't make a difference as we're using a fresh agent, but resetting the optimiser just to be safe. 
                self._agent.policy.optimizer = \
                    self._agent.policy.optimizer_class(
                        self._agent.policy.parameters(),
                        self._agent.policy.optimizer.defaults['lr'],
            ...
            if self.is_eval or agent_to_evaluate_file is not None:
            ...
            # load the previous session file or the agent to evaulate file
            if load_file:
                self._agent = self._agent_class.load(load_file, env=self._env)
            else:
                self._agent = self._agent_class.load(agent_to_evaluate_file, env=self._env)
